...
df[["acc_y", "label"]].boxplot(by="label", figsize=(20, 10), showfliers=True)
...
col = "gyro_y"
dataset = mark_outliers_chauvenet(df, col)
dataset[dataset["gyro_y_outlier"]] #boolean indexing to select outliers

#selection based on the boolean column created by the outlier detection method, here Chauvenet's criterion. This will give you all rows where the value in the "gyro_y_outlier" column is True, meaning those rows are identified as outliers for the "gyro_y" column. You can then decide how to handle these outliers, such as replacing them with NaN, removing them from the dataset.
dataset.loc[dataset["gyro_y_outlier"], "gyro_y"] = np.nan #replace outliers with NaN
dataset
